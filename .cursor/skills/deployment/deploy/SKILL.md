...
description: >-
  ...
  with QUICK_DEPLOY.sh <namespace> <cluster>, including building/pushing images and wiring the
  ingress DNS / /etc/hosts. Use to stand up or redeploy the retrieval side after build-yamls.
---

# Deployment: deploy retrieval (vss2)

Applies everything in `deployments/vss-k8s-application/` to the cluster. Prereq: `deploy-build-yamls` (secret + image tags), `kubectl` configured.
...
**Secret:** prefer [`team-configs/secrets/backend-secret.yaml`](../../../team-configs/secrets/backend-secret.yaml). If missing, ask the user to put it there (`team-configs/secrets/README.md`), then copy/symlink into `deployments/vss-k8s-application/backend-secret.yaml` for `QUICK_DEPLOY.sh`. Do not invent secret values.
...
```
<INGRESS_IP> video-lab.<cluster>.vastdata.com        # frontend + /api backend
```
...
1. Confirm `kubectl cluster-info`, images pushed, and `team-configs/secrets/backend-secret.yaml` exists (ask user to add it if not).
2. Ensure deploy dir has `backend-secret.yaml` (copy from `team-configs/secrets/` if needed).
3. Run `QUICK_DEPLOY.sh <ns> <cluster>`; capture the ingress IP.
4. Give the `/etc/hosts` lines; verify with `deploy-health`.
